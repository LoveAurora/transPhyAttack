...
class DecodeBox(nn.Module):
    ...
    def forward(self, input):
        ...
        grid_x = torch.arange(input_width).repeat(input_height, 1).repeat(batch_size * self.num_anchors, 1, 1).view(
            x.shape).type(FloatTensor)
        grid_y = torch.arange(input_height).repeat(input_width, 1).t().repeat(batch_size * self.num_anchors, 1, 1).view(
